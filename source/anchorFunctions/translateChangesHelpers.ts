...
export const userDeletedAnchor = (
    annotationId: string,
    anchorId: string
): null => {
    const affectedAnnotation: Annotation | undefined = annotationList.find(
        (a: Annotation) => a.id === annotationId
    )
    if (!affectedAnnotation) return null
    const updatedAnchors: AnchorObject[] = affectedAnnotation.anchors.filter(
        (a: AnchorObject) => a.anchorId !== anchorId
    )
    if (
        selectedAnnotationsNavigations.map((a) => a.anchorId).includes(anchorId)
    ) {
        setSelectedAnnotationsNavigations(
            selectedAnnotationsNavigations.filter(
                (nav) => nav.anchorId !== anchorId && nav.id !== annotationId
            )
        )
    }
...
export const userAutocompletedOrCommented = (
    changeText: string,
    textLength: number,
    rangeLength: number
): number => {
    const computedTabsize: number =
        typeof tabSize === 'number' ? tabSize : parseInt(tabSize)
    // USER DOES NOT ADD NEWLINES
    // checks for tab
    if (
        !insertSpaces &&
        (changeText.match(/\t/g) || []).length === textLength
    ) {
        // console.log('in not insertspaces if')
        textLength = textLength - rangeLength
    } else if (
        insertSpaces &&
        ((changeText.match(/\s/g) || []).length / computedTabsize) *
            computedTabsize ===
            textLength
    ) {
...
export const userChangedTextBeforeStart = (
    newRange: Anchor,
    ...
    textLength: number,
    rangeLength: number,
    anchorOnSameLine: boolean,
    changeHappenedOnBound: boolean
): Anchor => {
    const changeRangeOverlapsStart =
        originalAnchor.startOffset < changeRange.end.character &&
        originalAnchor.startOffset > changeRange.start.character

    // if user added text at the start offset, keep start offset
    if (
        changeHappenedOnBound &&
        (!isDeleteOperation || originalAnchor.startOffset - rangeLength < 0)
    ) {
    ...
    // if user deleted text at the start offset and its a one line anchor, move start character back and move end offset back
        ...
        changeHappenedOnBound &&
        isDeleteOperation &&
        changeRange.start.character === originalAnchor.startOffset &&
        changeRange.end.character > originalAnchor.startOffset &&
        anchorOnSameLine
    ) {
    ...
    // move back start and optionally change end
        ...
        if (anchorOnSameLine) {
            newRange.endOffset =
                originalAnchor.endOffset -
                originalAnchor.startOffset -
                (changeRange.end.character - originalAnchor.startOffset) +
                changeRange.start.character
        }
    }
    // simplest case - either remove length of deletion or add length of text
    else {
        newRange.startOffset = isDeleteOperation
            ? originalAnchor.startOffset - rangeLength
            : originalAnchor.startOffset + textLength
    }
    // do same thing for end
    if (anchorOnSameLine && !changeRangeOverlapsStart) {
        newRange.endOffset = isDeleteOperation
            ? originalAnchor.endOffset - rangeLength
            : originalAnchor.endOffset + textLength
    }
...
// update end offset
export const userChangedTextBeforeEnd = (
    newRange: Anchor,
    originalAnchor: Anchor,
    isDeleteOperation: boolean,
    textLength: number,
    ...
    newRange.endOffset = isDeleteOperation // if delete
        ? originalAnchor.endOffset >= changeRange.end.character // and if the end of our anchor is past where the edit ends
            ...
            : originalAnchor.endOffset -
              (originalAnchor.endOffset - changeRange.start.character) // else remove the amount of code that was originally within our anchor point
...
export const userChangedLinesBeforeStart = (
    newRange: Anchor,
    originalAnchor: Anchor,
    numLines: number
): Anchor => {
...
export const userChangedLinesInMiddle = (
    newRange: Anchor,
    originalAnchor: Anchor,
    changeRange: vscode.Range,
    numLines: number,
    ...
    if (!anchorOnSameLine) {
        ...
        if (changeRange.start.line === originalAnchor.startLine) {
            ...
            newRange = userChangedLinesInMiddleUpdateStart(
                newRange,
                originalAnchor,
                changeRange,
                numLines,
        ...
        // change started at the end of the anchor
            ...
            changeRange.start.line === originalAnchor.endLine &&
            changeRange.start.character >= originalAnchor.endOffset
        ) {
            // console.log('userChangedLinesInMiddleUpdateEndUsingStart');
            newRange = userChangedLinesInMiddleUpdateEndUsingStart(
                newRange,
                originalAnchor,
                changeRange,
                numLines,
        ...
        // change ended at the end of the anchor
            ...
            changeRange.end.line === originalAnchor.endLine &&
            originalAnchor.endOffset >= changeRange.end.character
        ) {
            // console.log('userChangedLinesInMiddleUpdateEndUsingEnd');
            newRange = userChangedLinesInMiddleUpdateEndUsingEnd(
                newRange,
                changeRange,
                numLines,
                anchorText,
...
const userChangedLinesInMiddleUpdateStart = (
    newRange: Anchor,
    originalAnchor: Anchor,
    changeRange: vscode.Range,
    numLines: number,
    ...
    // set new start line
    newRange.startLine =
        originalAnchor.startLine - numLines < 0 // error check
            ? 0
    ...
    const relevantTextLength: number = changeText
        .substring(changeText.lastIndexOf('\n') + 1)
        .replace('\n', '')
        .replace('\r', '').length
    const originalStart: number = newRange.startOffset,
        originalEnd: number = newRange.endOffset
    newRange.startOffset =
        changeRange.start.character === originalStart
            ? relevantTextLength
            : originalAnchor.startOffset -
              changeRange.start.character +
              relevantTextLength
    ...
    if (
        originalRange.start.isEqual(changeRange.start) &&
        originalRange.end.isBefore(changeRange.end) &&
        !changeText.length
    ) {
        ...
        const didChangeTurnMultilineAnchorIntoOneLine: boolean =
            originalAnchor.endLine + numLines === newRange.startLine // we have already added/subtracted lines to endLine
        if (didChangeTurnMultilineAnchorIntoOneLine) {
            newRange.endOffset =
                changeRange.end.line === originalAnchor.endLine &&
                changeRange.end.character < originalAnchor.endOffset
                    ? changeRange.start.character + originalAnchor.endOffset
                    : changeRange.start.character
            newRange.endLine = changeRange.start.line
        } else {
            newRange.endOffset = anchorText.substring(
        ...
        anchorOnSameLine &&
        originalStart <= changeRange.start.character
    ) {
        newRange.endOffset =
            doesChangeTextIncludeNewLine &&
            originalEnd > changeRange.start.character &&
            originalStart < changeRange.start.character
                ? anchorLength -
                  anchorText.substring(
                      anchorLength - changeRange.start.character
                  ).length +
                  relevantTextLength
                : newRange.startOffset + anchorLength
        newRange.endLine =
            originalEnd > changeRange.start.character &&
            doesChangeTextIncludeNewLine
                ? newRange.endLine + numLines
                : newRange.endLine
    } else if (
        doesChangeTextIncludeNewLine &&
        doesAnchorIncludeNewline &&
        newRange.endLine === changeRange.start.line
    ) {
        // console.log('update last line')
        const lastStr: string =
            anchorText.substring(anchorText.lastIndexOf('\n')) + 1
        newRange.endOffset = lastStr.substring(
            lastStr.length - changeRange.start.character
        ).length
    } else if (
        doesChangeTextIncludeNewLine &&
        anchorOnSameLine &&
        ...
        newRange.startOffset =
            relevantTextLength + (originalStart - changeRange.start.character)
...
const userChangedLinesInMiddleUpdateEndUsingStart = (
    newRange: Anchor,
    originalAnchor: Anchor,
    changeRange: vscode.Range,
    numLines: number,
    ...
    if (numLines > 0) {
        ...
        const pointAtWhichAnchorIsSplit: number =
            originalAnchor.endOffset - changeRange.start.character
        const originalPositionEqualToEnd = new vscode.Position(
            originalAnchor.endLine,
            originalAnchor.endOffset
        ).isEqual(changeRange.end)
        newRange.endOffset = originalPositionEqualToEnd
            ? changeRange.end.character
            : anchorText.includes('\n')
            ? anchorText.substring(anchorText.lastIndexOf('\n')).length -
              anchorText.substring(
                  anchorText.lastIndexOf('\n') + 1,
...
const userChangedLinesInMiddleUpdateEndUsingEnd = (
    newRange: Anchor,
    changeRange: vscode.Range,
    numLines: number,
    anchorText: string,
    ...
    if (numLines < 0) {
        const shrinkingIntoOneLineAnchor: boolean =
            originalAnchor.endLine + numLines === originalAnchor.startLine
        ...
        numLines > 0 &&
        changeRange.start.line === originalAnchor.endLine &&
        changeRange.start.isEqual(changeRange.end)
    ) {
        ...
        if (
            ...
            newRange.startOffset =
                newRange.endOffset -
                (originalEndOffset - originalAnchor.startOffset)
        }
    }
    newRange.endLine = originalAnchor.endLine + numLines
...
export const shrinkOrExpandBackOfRange = (
    newRange: Anchor,
    changeRange: vscode.Range,
    numLines: number,
    changeText: string,
    ...
    if (
        ...
        numLines &&
        !changeText.length &&
        changeRange.start.isEqual(originalRange.end)
    ) {
...
export const shrinkOrExpandFrontOfRange = (
    newRange: Anchor,
    changeRange: vscode.Range,
    numLines: number,
    changeText: string,
    ...
    if (numLines && !changeText.length) {
        const diff: number = sameLineAnchor
            ? newRange.endOffset -
              (changeRange.end.character - newRange.startOffset)
            : anchorText.substring(anchorText.lastIndexOf('\n')).length -
              (changeRange.end.character - newRange.startOffset)
        ...
        newRange.endOffset =
            sameLineAnchor || newRange.startLine + numLines === newRange.endLine
                ? changeRange.start.character + diff - newRange.startOffset
                : newRange.endOffset
        newRange.startLine = changeRange.start.line
        newRange.startOffset =
            (sameLineAnchor ||
                newRange.startLine + numLines === newRange.endLine) &&
            rangeLength > originalStartOffset &&
            !isOffsetBetweenChangeRangeEndAndAnnoStart
                ? changeRange.start.character
                : changeRange.start.character +
                  originalStartOffset -
                  changeRange.end.character
        ...
        newRange.startLine =
            ...
            changeRange.start.character <= originalStartOffset
                ? changeRange.start.line + numLines
                : changeRange.start.line
        newRange.startOffset = isOffsetBetweenChangeRangeEndAndAnnoStart
            ? changeRange.start.character +
              (originalStartOffset - changeRange.end.character) +
              changeText.length
            : changeRange.start.character
        newRange.endOffset = sameLineAnchor
            ? newRange.startOffset + anchorText.length
            : newRange.endOffset
        newRange.endLine =
            changeRange.start.line === originalStartLine &&
            changeRange.start.character <= originalStartOffset
                ? newRange.startLine + numLines
                : newRange.endLine
    }
